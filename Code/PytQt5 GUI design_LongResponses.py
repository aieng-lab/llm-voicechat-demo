...
class PyShine_LIVE_PLOT_APP(QtWidgets.QMainWindow):
    def __init__(self, params=params):
        ...
        self.responses.append(self.welcome_message)
        self.ttt_model.conv.append_message(self.ttt_model.conv.roles[1], self.welcome_message)
        self.filepath = self.generate_audio()
        self.filepath = "text_to_speech.wav"
    ...
    def record_speech(self):
        ...
        # self.query = "Kannst du mir eine kurze Geschichte erzählen?\n"
        print(self.query)
    ...
    def query_vicuna(self, query:str):
        ...
        for outputs in self.ttt_model.run(query):
            self.responses.append(outputs)
            print(outputs)
        self.query_allowed = False
    ...
    def generate_audio(self):
        ...
        self.filepath = self.tts_model.run_ex_2(self.responses)
        self.responses=[]
        # self.filepath = "text_to_speech.wav"
        self.generating_allowed = False
        self.speaking_allowed=True
    ...
    def start_worker_3_tts(self, emit_sig):
        if emit_sig ==1 and self.generating_allowed and self.stopped is False:
            ...
            tts_worker = TTSWorker(self.generate_audio)
    ...
    def start_worker_4_speaker(self):
        if self.speaking_allowed and not self.stopped:
